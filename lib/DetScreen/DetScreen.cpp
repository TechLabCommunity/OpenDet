...
DetScreen::DetScreen(uint paddress, uint pcols, uint prows) : LiquidCrystal_I2C(paddress, pcols, prows){
  if (pcols == 0)
    SYSERR("DetScreen must have 1 column at least.");
  if (prows == 0)
    SYSERR("DetScreen must have 1 row at least.");
  address = paddress;
  cols = pcols;
  rows = prows;
}
...
String DetScreen::cents_to_string(const uint& cents){
  ...
  if (cents < 100){
    for (uint i=0; i<4-cc.length(); i++)
      cc = "0"+cc;
  }
  return cc.substring(0, cc.length()-2)+"."+cc.substring(cc.length()-2, cc.length());
}
...
void inline DetScreen::smartprint(const String& s, uint row){
...
void DetScreen::start_system(const String& version){
  ...
  delay(1000);
  smartprint("Starting  system", 1);
  delay(1000);
  smartprint("TechLab  Community", 3);
  delay(2000);
}
